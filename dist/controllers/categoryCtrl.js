...
const categoryCtrl = {
    ...
    updateCategory: (req, res) => __awaiter(void 0, void 0, void 0, function* () {
        ...
        if (req.user.role !== 'admin')
            return res.status(400).json({ msg: "Invalid Authentication!" });
        try {
            yield categoryModel_1.default.findOneAndUpdate({ _id: req.params.id }, {
                name: (req.body.name).toLowerCase()
            });
            res.json({ msg: "Success! Category updated" });
        }
        catch (err) {
            return res.status(500).json({ msg: err.message });
        }
    ...
    deleteCategory: (req, res) => __awaiter(void 0, void 0, void 0, function* () {
        ...
        try {
            ...
            if (blog)
                return res.status(400).json({ msg: "This category contain blogs, cannot delete!" });
